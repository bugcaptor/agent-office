...
APP="${1:-src-tauri/target/release/bundle/macos/agent-office.app}"
...
다음 단계:
  1) 앱을 설치한다:  cp -R "$APP" /Applications/
  2) 애드혹 시절의 깨진 TCC 레코드를 청소한다:
       tccutil reset All $BUNDLE_ID
  3) 앱을 실행하고 프롬프트에 서비스별로 한 번씩만 답한다.
     사진 / 미디어 라이브러리 / 이동식·네트워크 볼륨 -> 거부해도 기능 손실 없음
