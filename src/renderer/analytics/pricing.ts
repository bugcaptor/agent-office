...
/**
 * 토큰 셀 표기 — `11.7K(1.7M)`처럼 **순수 입출력(괄호 밖)과 캐시 포함 전체
 * (괄호 안)를 병기**한다. 요약 바와 분석 패널이 같은 함수를 쓴다(두 화면이
 * 같은 숫자를 다르게 그리지 않게).
 *
 ...
 *
 * - 전체가 0이면 `—`(집계된 토큰이 없다).
 * - 캐시가 없어 둘이 같으면 괄호를 붙이지 않는다 — 같은 숫자를 두 번 쓰는
 *   셈이라 폭만 먹는다.
 * - 캐시만 있는 턴은 `0(500)`으로 정직하게 그린다. 예전에 이걸 `—`로 떨궜던
 *   건 캐시가 셀에서 통째로 안 보였기 때문이고, 지금은 괄호가 그 값을 보여준다.
 *
...
export function formatTokenPair(net: number, gross: number): string {
  ...
  return `${formatTokens(net)}(${formatTokens(gross)})`;
}
