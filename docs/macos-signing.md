...
- **개인키를 백업해라.** 키를 잃으면 새 인증서 = 새 DR = TCC 프롬프트가 한 번 더 돈다. 키체인 접근.app에서 `.p12`로 내보내 보관하면 된다.
...
자체 서명은 **Gatekeeper를 만족시키지 못한다.** 이건 유료 멤버십 없이는 우회로가 없다 — 공증(notarization)은 Apple Developer Program 회원에게만 열린 서비스다.

### DMG를 받아 쓰는 사람

배포본도 자체 서명으로 굽는 것이 **애드혹보다 낫다.** 인증서를 고정해 두면 DR(`certificate root = H"…"`)이 릴리스마다 동일하게 유지되어, 사용자는 앱을 업데이트해도 권한을 다시 묻지 않는다. 애드혹으로 뿌리면 릴리스마다 다시 묻는다.

릴리스 DMG는 이름 기반 조회를 쓰므로 인증서에 **신뢰 설정이 걸려 있어야 한다**:

```bash
APPLE_SIGNING_IDENTITY="Agent Office Local Signing" npm run tauri build
```

사용자에게는 첫 실행 시 Gatekeeper 우회를 안내해야 한다:

> 시스템 설정 → 개인정보 보호 및 보안 → 아래로 스크롤 → **"확인 없이 열기"**

macOS 15부터 우클릭→열기 우회가 막혀 이 경로만 남았다. 터미널을 선호하면 `xattr -dr com.apple.quarantine /Applications/agent-office.app`.
