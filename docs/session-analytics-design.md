...
형식은 `11.7K(1.7M) · $0.42` — 토큰은 괄호 밖에 `input+output`, 괄호 안에 캐시까지
포함한 전체를 `formatTokens`로 축약해 병기한다(분석 패널과 같은 규칙, 같은 함수
...
- ~~**캐시만 있는 턴**(`input + output === 0`인데 `cacheRead > 0`) — 토큰 자리를
  ...
  뒤로는 `0(500)`으로 그린다 — `—`로 떨궜던 건 캐시가 셀에서 통째로 안 보이던
...
**고친 것**: 토큰 셀을 `11.7K(1.7M)`처럼 병기로 바꿨다. 괄호 밖은 순수 입출력,
괄호 안은 캐시까지 포함한 전체이고, **괄호 안 숫자가 비용 셀과 같은 기준**이다.
전체 합 하나로 갈아치우지 않고 병기로 둔 건 "내가 실제로 시킨 일의 크기"(입출력)와
"과금 기준"(전체)이 서로 다른 질문에 답하기 때문이다.
...
- 캐시가 없어 두 값이 같으면 괄호를 안 붙인다. 같은 숫자를 두 번 쓰면 폭만 먹는다.
- 전체가 0일 때만 `—`. 캐시만 있는 턴은 `0(500)`으로 정직하게 그린다(§11.6의
  세 번째 강등 규칙 폐기).
