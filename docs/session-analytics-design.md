...
형식은 `11.7K CH(1.7M) · $0.42` — 순수 입출력(`input+output`) 뒤에 공백과 `CH`
표식, 그 뒤 괄호 안에 캐시까지 포함한 전체를 `formatTokens`로 축약해 병기한다
(분석 패널과 같은 규칙, 같은 함수
`pricing.formatTokenPair`). 항목별 내역(입력/출력/캐시읽기/캐시쓰기/턴 수/대표
모델 + "공개 API 요율 기준 추정치" 각주)은 `title` 툴팁에 그대로 있다. **병기로
바뀐 경위는 §11.8** — 원래는 괄호 없이 `input+output`만 그렸다. `costUnknownTurns > 0`
...
- ~~**캐시만 있는 턴**(`input + output === 0`인데 `cacheRead > 0`) — 토큰 자리를
  `0`이 아니라 `—`로 보여준다.~~ **§11.8에서 폐기.** 괄호가 캐시를 보여주게 된
  뒤로는 `0 CH(500)`으로 그린다 — `—`로 떨궜던 건 캐시가 셀에서 통째로 안 보이던
  시절의 규칙이다. 지금 `—`는 전체가 0일 때만.

(참고: `AnalyticsDialog`에도 "비용을 하나도 모를 때" 문제가 원래부터 있지만,
그 컴포넌트는 이번 §11 확장 범위 밖이라 손대지 않았다. 토큰 열 병기는 §11.8에서
양쪽에 같이 넣었다.)
...
## 11.8 토큰 셀을 순수 입출력과 전체 병기로 바꿨다 (2026-08-31)

**결론부터: 요약 바와 분석 패널의 토큰 숫자가 실제의 1% 남짓만 보여주고 있었다.**
둘 다 `input + output`만 그렸는데, 프롬프트 캐시가 켜진 요즘 세션에서는 그 둘이
전체의 1%도 안 된다. 이 문서를 쓰는 세션의 전사를 직접 세어 보면 이렇다.

| 항목 | 토큰 |
|---|---|
| input | 44 |
| output | 11,659 |
| cache read | 1,557,532 |
| cache write | 107,498 |
| 셀에 뜨던 값 (`input+output`) | **11.7K** |
| 실제 총합 | **1.68M** |

표시값이 실제의 **0.7%**다. 사용자가 "실시간 사용량이 실제보다 적게 나온다"고
느낀 게 정확히 이것이었다 — 수집이 새는 게 아니라 **표시 기준이 캐시를 빼고
있었다**.

기준이 어긋난 자리가 하나 더 있었다. 바로 옆 비용 셀은 `estimateCostUsd`가
캐시 읽기·쓰기까지 넣어 계산한다(§9.4). 그러니까 한 줄 안에서 토큰은 11.7K인데
비용은 1.68M어치를 물리고 있었다. 두 셀이 서로 다른 것을 세고 있었던 셈이다.

**고친 것**: 토큰 셀을 `11.7K CH(1.7M)`처럼 병기로 바꿨다. `CH` 앞은 순수
입출력, 괄호 안은 캐시까지 포함한 전체이고, **괄호 안 숫자가 비용 셀과 같은
기준**이다. 전체 합 하나로 갈아치우지 않고 병기로 둔 건 "내가 실제로 시킨
일의 크기"(입출력)와 "과금 기준"(전체)이 서로 다른 질문에 답하기 때문이다.

- 표기 함수는 `renderer/analytics/pricing.ts`의 `formatTokenPair(net, gross)`
  하나로 두고 `TerminalSummaryBar`와 `AnalyticsDialog`가 같이 쓴다 — 같은 숫자를
  두 화면이 다르게 그리는 일을 구조적으로 막는다.
- 캐시가 없어 두 값이 같으면 `CH` 표식도 괄호도 안 붙인다. 같은 숫자를 두 번
  쓰면 폭만 먹는다.
- 전체가 0일 때만 `—`. 캐시만 있는 턴은 `0 CH(500)`으로 정직하게 그린다
  (§11.6의 세 번째 강등 규칙 폐기).
- 괄호의 뜻은 툴팁 한 줄(`tokenPairHint`)로 밝힌다. 괄호가 붙었을 때만 붙이고,
  키는 terminal·activity 두 네임스페이스의 여섯 로케일에 모두 넣었다.
- **표기 미세 조정(같은 날)**: 처음엔 `11.7K(1.7M)`처럼 괄호를 숫자에 바로
  붙였는데, 두 숫자가 시각적으로 뭉쳐 읽기 어렵다는 지적을 받았다. 순수
  입출력 뒤에 공백을 넣고 캐시(Cache)를 뜻하는 `CH` 표식을 얹어 `11.7K
  CH(1.7M)`로 바꿨다 — 병기의 뜻·강등 규칙은 그대로이고 시각적 구분만
  또렷해졌다.

**비교 기준 주의**: 이 값을 CLI의 `/context`와 대보면 안 맞는 게 정상이다.
`/context`는 누계가 아니라 **지금 컨텍스트 창을 얼마나 쓰고 있는지**(마지막 턴
한 번의 입력 크기)를 보여준다. 이 세션 기준으로 `/context`는 약 107K인데 누계는
1.68M이다 — 서로 다른 것을 재는 숫자다. 구독 한도 대비 사용률을 보고 싶으면
BottomBar의 사용량 게이지(`docs/usage-design.md`)를 봐야 한다.
...
어댑터 한정) 중간 사용량을 실어 이 지연을 없앤다.

**왜 claude 어댑터에서 공짜로 되는가**: `ClaudeAdapter::turn_usage`(§9.2)는
이미 워터마크 델타 추출이다 — 파일 크기·`message.id`가 그대로면 스캔 자체를
건너뛰고(`usage_for_path`), 늘었으면 **직전 스캔 이후의 증분만** 센다. Stop이
쓰던 이 함수를 PostToolUse에서도 그대로 부르면, 같은 구간을 두 번 세는 일 없이
"지금까지"를 여러 번 나눠 실을 수 있다 — 이중 계산 걱정이 새로 생기지 않는다.

**어떻게 얹었나**:

- `map_post_tool_use`가 이미 갖고 있던 5초 `progress_due` 스로틀(내레이션 tail
  읽기용, §이슈 #43) 위에 그대로 얹는다 — 도구마다 전사를 다시 읽지 않고, 내레이션과
  같은 예산 안에서 `self.turn_usage(transcript_path)`를 부른다. 서브에이전트
  내부 도구(agent_id 있음)도 라벨 텍스트는 여전히 안 싣지만 tokens는 싣는다 —
  그 갈래의 `transcript_path`는 서브 자신의 `subagents/*.jsonl`을 가리키므로
  (§9.2), 메인 세션의 워터마크와 충돌하지 않는다.
- `ObserverEvent::Tool`에 `tokens: Option<SessionEventTokens>`를 더했다. codex/pi
  는 이 기능이 없어(codex는 rollout 턴-경계 델타뿐, pi는 전사/rollout 자체가
  없음) 항상 `None`이다.
- `notification/hub.rs`의 Tool 팔이 tokens가 있으면 Stop과 같은 `emit_turn_usage`
  헬퍼로 즉시 `TurnUsageEvent`를 낸다 — 알림 게이트·dedup과 무관한 독립 채널이라는
  §11.1의 성질을 그대로 물려받는다(Tool은 애초에 알림을 안 낸다).

**`TurnUsageEvent.partial`**: 이 갱신이 "턴이 아직 안 끝났다"는 뜻을 실어야
하는 이유는 소비자(`sessionCost.addTurn`)가 이벤트 개수로 "턴이 몇 번 있었는지"를
세기 때문이다. 한 턴 안에서 도구를 세 번 부르면 partial 이벤트가 세 번 오는데,
그때마다 턴을 하나씩 잡으면 "도구 세 번"이 "턴 세 번"으로 둔갑한다. 그래서
`TurnUsageEvent`(wire)·`SessionEventRecord`(시계열, kind=Usage)에 `partial: bool`
(전자)/`Option<bool>`(후자, 하위호환용 옵션 추가라 schemaVersion은 1을 유지)을
더했다. Stop은 `partial: false`, PostToolUse는 `partial: true`.

- `addTurn(totals, tokens, partial = false)`: partial이면 토큰·비용은 그대로
  더하되(그만큼 실제로 썼으므로) **`turns`도 `costUnknownTurns`도 올리지 않는다**.
  그 턴의 최종 Stop이 `partial:false`로 다시 잡히면서 그때 비로소 턴 하나로
  세고, 단가를 몰랐다면 그 시점에 `costUnknownTurns`도 오른다. 드물게 그 턴의
  마지막 도구 이후 usage 줄이 하나도 없어 Stop의 델타가 완전히 비면(→
  `tokens: None`, `addTurn` 자체가 안 불림) 그 턴은 단가 미상 표시가 통째로
  빠질 수 있다 — 실사용에서 관측되면 그때 다룬다.
- `aggregateSeed`(부팅 시드)도 `record.partial ?? false`를 그대로 넘긴다 — 이
  필드가 생기기 전의 과거 레코드는 전부 Stop 유래(=턴이 끝난 것)이므로 없으면
  `false`로 강등해야 한다. 반대로 강등하면(없으면 true로 본다) 과거 파일의
  턴 수가 전부 유실된다.
- `RecordingAppEvents::turn_usage`가 kind=Usage 레코드에 `partial`을 그대로
  적는다 — 안 적으면 재부팅 후 시드가 이 레코드를 항상 "턴이 끝난 것"으로
  오인해 turns가 실제보다 부풀 수 있다(위 규칙의 반대 방향 실패).

**분석 패널(§9)에는 손대지 않았다**: `aggregate.ts`의 일별 집계는 턴 수를
`reconstructTurns`(프롬프트~Stop 경계)로 따로 세지, kind=Usage 레코드 개수로
세지 않는다 — 그래서 이 확장이 새 kind=Usage 레코드를 더 자주 만들어도 분석
패널의 "턴" 열은 부풀지 않는다. 다만 `costUnknownTurns`(그 표의 같은 이름의
집계)는 **이벤트 단위**로 세므로, 단가를 모르는 모델이 한 턴 안에서 중간
갱신을 여러 번 냈다면 그 열의 카운트가 실제 턴 수보다 커질 수 있다 — 요약 바
