...
2. 이름, 입력 변수, 작업 폴더, 단계, 반복 횟수를 편집한다.
...
AutomationDefinition {
  ...
  workspace?: string, inputValues?: { [key]: string },
...
정의는 앱 데이터 폴더의 `automations/`에 JSON으로 원자 저장한다. 저장 버튼은 편집한 작업 폴더(`workspace`)와 실행 입력값(`inputValues`)도 함께 보관하며 다시 선택하거나 편집기를 다시 열 때 복원한다. 입력 변수의 `default`는 템플릿 기본값으로 별도 유지한다. 두 필드가 없는 기존 정의는 현재 탭의 작업 폴더와 변수 기본값을 사용한다. 작업 폴더나 실행 입력값만 바꾸어도 미저장 변경으로 처리한다. 실행은 저장한 설정을 바탕으로 작업 폴더와 실행 입력값을 확정한다. 작업 폴더는 절대경로로 고정하며 셸의 현재 위치를 바꾸지 않는다. 사용자가 셸에서 수행한 `cd`와 세션 생성 시 작업 폴더는 다를 수 있다.
