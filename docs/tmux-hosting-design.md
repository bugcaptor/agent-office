...
살아있던 tmux 세션에 다시 붙는 일은 하지 않는다. 소환할 때마다 새 이름·새
sid로 새 세션을 만든다.
...
```
ao-<슬러그>-<agentId 앞 6자>-<sid 앞 6자>
```

**슬러그만으로는 안 된다.** 슬러그는 캐릭터 이름에서 나오므로 유일하지
않다 — 이름이 같은 캐릭터가 둘이면 A를 소환할 때 B의 세션을 gc가 죽인다.
순한글 이름은 ASCII 슬러그가 통째로 비어 전부 같은 폴백으로 몰린다. 그래서
`agentId` 조각이 들어가야 gc가 "그 캐릭터 것만" 건드리게 된다.

슬러그 규칙: 소문자화 → `[a-z0-9]` 외 전부 `-` → 연속 `-` 압축 → 앞뒤 `-`
제거 → 최대 12자. 비면 `"agent"`. tmux에서 문제가 되는 `.` `:`는 이 필터가
자동으로 걷어낸다.

프로필에서 고정 이름을 받지 않는다 — 충돌을 만들고, "이미 있으면 붙기"로
되돌아가고 싶어지게 만든다. **밖에서 이 세션을 찾을 때 기댈 수 있는 건
`ao-` 접두어뿐이고, 그게 사용자와 맺는 유일한 약속이다.** 나머지(슬러그·
agentId·sid 조각)는 내부 정리용이지 사용자가 파싱해서 쓰라고 만든 형식이
아니다.

구현: `tmux_host::slug`, `tmux_host::gc_prefix`, `tmux_host::session_name`.

## 5. `-t`는 `=` 정확일치를 쓴다

tmux `-t`는 접두/fnmatch 매칭이다 — `ao-nova-ab12cd`가
`ao-nova-ab12cdef`를 잡을 수 있다. gc·kill·attach가 전부 엉뚱한 세션을
...
| "다시 띄우기" | 죽이고 새 이름·새 sid로 다시 만든다(재접속 아님) |
