...
- **머리 위 작업 라벨** — 각 캐릭터 위에 프로젝트·목표·현재 작업이 두 줄로
  표시됩니다. 로컬 `claude` CLI로 요약할 수도 있습니다(옵트인).
- **세션 시간 집계** — 프롬프트 입력부터 응답 완료까지, 세션별 작업/대기 시간을
  패널로 보여줍니다(옵트인, Claude Code 훅 기반).
...
Claude Code 연동 기능은 전부 기본 **꺼짐**이며, 첫 실행 시 동의 다이얼로그에서
선택하고 이후 하단 바의 ⚙ 설정에서 언제든 바꿀 수 있습니다.

- **작업 라벨 요약 (claude CLI)** — 머리 위 라벨 요약에 로컬 `claude` CLI를
  호출합니다. 호출마다 Claude 구독 크레딧을 소모합니다.
- **알림·시간측정 (Claude Code 훅)** — 앱이 여는 세션의 `claude`에 훅 설정을
  주입하고 `127.0.0.1` 로컬 서버로 활동 이벤트를 받습니다. 꺼져 있으면 느낌표
  알림과 세션 시간측정이 동작하지 않습니다. 변경은 새 세션부터 적용됩니다.

연동을 끄더라도 터미널 세션 관리·캐릭터·테마 등 나머지 기능은 그대로 동작합니다.
